    ...
    <div x-data="{ 
        ...
        @if(count($projects) > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
                @foreach($projects as $project)
                    <div x-data="{ active: false }"
                        ...
                        :class="{ 'border-primary-dark/30 -translate-y-2 shadow-[0_0_30px_rgba(13,226,130,0.1)]': active, 'border-white/5': !active }"
                        class="group relative bg-[#0d0d0d] border rounded-2xl overflow-hidden transition-all duration-500 shadow-2xl flex flex-col h-full animate-fade-in-up stagger-1">
                        ...
                        <div class="relative aspect-video overflow-hidden border-b border-white/5">
                            ...
                            <div
                                class="absolute inset-0 bg-black/70 flex items-center justify-center backdrop-blur-sm shadow-inner transition-all duration-500"
                                :class="active ? 'opacity-100' : 'opacity-0 pointer-events-none'">
                                <a href="{{ route('projects.show', $project) }}"
                                    class="flex items-center gap-2.5 text-white font-medium text-xl tracking-tight transform transition-all duration-500 hover:scale-105 active:scale-95"
                                    :class="active ? 'translate-y-0' : 'translate-y-4'">
                                    <span class="opacity-90">View Project</span>
                                    <svg class="w-6 h-6 opacity-80 transition-transform" 
                                        :class="{ 'translate-x-1': active }"
                        ...
                        <div class="p-5 flex flex-col flex-1">
                            <div class="flex-1">
                                <h3
                                    class="text-xl font-black text-primary-dark mb-2 uppercase tracking-wide transition-colors duration-500">
                                    {{ $project->title }}</h3>
                                <p class="text-[13px] text-gray-400 leading-relaxed line-clamp-2 mb-4 font-medium">
                            ...
                            <div class="flex items-center justify-between mt-auto pt-4 border-t border-white/5">
                                    ...
                                    @if($project->tech_stack)
                                        @foreach($project->tech_stack as $tech)
                                            <div class="group/icon">
                                                <i class="{{ $tech }} colored text-lg transition-transform duration-300 group-hover/icon:scale-110"
                                                    title="{{ ucfirst(explode('-', $tech)[1] ?? $tech) }}"></i>
                                            </div>
                                        @endforeach
                                    @endif
                                </div>
                                ...
                                <div x-data="{ 
                                    ...
                                    class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/5 hover:border-yellow-500/30 transition-all cursor-pointer select-none group/heart focus:outline-none focus:ring-2 focus:ring-yellow-500/50"
                                    @click.stop="toggle()"
                                    role="button"
                                    tabindex="0"
                                    ...
                                    <span
                                        class="text-[11px] font-black text-gray-500 group-hover/heart:text-white transition-colors"
                                        x-text="count"></span>
                                    <svg class="w-4 h-4 text-yellow-500 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)] transition-transform group-active:scale-125"
            ...
            @if(count($projects) > 4)
                <div class="mt-12 flex justify-center">
                    <button @click="showAll = !showAll" aria-label="Toggle projects visibility"
                        ...
                        <svg class="relative z-10 w-3.5 h-3.5 transition-transform duration-500"
                            :class="showAll ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" />
                        </svg>
                        <!-- Inner Glow (Green) -->
                        <div
                            class="absolute inset-0 rounded-full bg-primary-dark/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
                        </div>
                    </button>
                </div>
            @endif
        @else
            <div class="text-center py-20 bg-[#0d0d0d] rounded-xl border border-dashed border-border text-gray-400">
                <svg class="w-16 h-16 mx-auto mb-4 text-gray-800 opacity-50" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1"
